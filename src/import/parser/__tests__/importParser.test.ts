...
describe('parseImportRows', () => {
  ...
  it('productId existe en productMaster → válido', () => {
    const rows: RawImportRow[] = [
      { warehouse: 'ELITE MIAMI', productId: 'ABC123', quantity: 5 }
    ]
  ...
  it('productId NO existe → inválido con reason INVALID_PRODUCT', () => {
    const rows: RawImportRow[] = [
      { warehouse: 'ELITE MIAMI', productId: 'ZZZ999', quantity: 5 }
    ]
    const result = parseImportRows(rows, productMaster)
  ...
  it('row válida', () => {
    const rows: RawImportRow[] = [
      { warehouse: ' ELITE MIAMI ', productId: 'ABC123', quantity: 10 }
    ]
    const result = parseImportRows(rows, productMaster)
    expect(result.valid.length).toBe(1)
    expect(result.invalid.length).toBe(0)
    expect(result.valid[0].warehouseId).toBe('WH-006')
    expect(result.summary.totalRows).toBe(1)
    expect(result.summary.validCount).toBe(1)
    expect(result.summary.invalidCount).toBe(0)
  })
  ...
  it('productId vacío → inválido', () => {
    const rows: RawImportRow[] = [
      { warehouse: 'ELITE MIAMI', productId: '', quantity: 10 }
    ]
    const result = parseImportRows(rows, productMaster)
    expect(result.valid.length).toBe(0)
    expect(result.invalid.length).toBe(1)
  ...
  it('quantity <= 0 → inválido', () => {
    const rows: RawImportRow[] = [
      { warehouse: 'ELITE MIAMI', productId: 'ABC123', quantity: 0 }
    ]
    const result = parseImportRows(rows, productMaster)
    expect(result.valid.length).toBe(0)
    expect(result.invalid.length).toBe(1)
